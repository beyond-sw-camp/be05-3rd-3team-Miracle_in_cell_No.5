  ...
  <div class="roomInfo-container">
    <div class="roomInfo">
      <div class="submit-post">
        ...
        <div class="form-row">
          <div class="form-group category-buttons">
            ...
            <div class="category-button-group">
              ...
              <button :class="{ active: post.category === 'COMPLIMENT' }" @click="setCategory('COMPLIMENT')">
                Compliment
              </button>
              <button :class="{ active: post.category === 'COMPLAIN' }" @click="setCategory('COMPLAIN')">Complain
              </button>
            </div>
          </div>
          <div class="form-group anonymous-buttons">
            ...
            <div class="anonymous-button-group">
              <button :class="{ active: post.anonymous === 'Realname' }" @click="setAnonymous('Realname')">닉네임 공개
              </button>
              <button :class="{ active: post.anonymous === 'Anonymous' }" @click="setAnonymous('Anonymous')">닉네임 비공개
              </button>
            </div>
          </div>
        </div>
        <div class="form-group">
          <label>내용</label>
          <input name="postContent" id="postContent" v-model="post.content" type="text" placeholder="내용을 입력하세요."/>
        </div>
...
export default {
  setup() {
    ...
    const submitPost = async () => {
      try {
        // API를 호출하여 사용자가 입력한 내용을 서버에 전송
        await postApi.savePostById(roomId, post.value);
        console.log("Post saved successfully!");
        // 저장 후 필요한 작업 수행
        // 예: 저장 후 화면을 다시 로드하거나 특정 경로로 이동
      } catch (error) {
